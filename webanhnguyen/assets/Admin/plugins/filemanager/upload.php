...
if ( ! empty($_FILES))
{
	...
	if (in_array(fix_strtolower($info['extension']), $ext))
	{
		...
		$pos_product = strpos($targetFile, '../../../../images/product');
		$pos_news = strpos($targetFile, '../../../../images/posts');
		if (($pos_product !== false && $pos_product == 0) || ($pos_news !== false && $pos_news == 0)) {
		   ...
		   $watermarkImage = '../../../../images/'. $_SESSION['RF']['watermark']['image_watermark'];
		   if($_SESSION['RF']['watermark']['image_watermark'] != '' && file_exists($watermarkImage)){
				...
				$magicianObj->addWatermark($watermarkImage, $position, $padding = 0, $opacity = 0);
		   }
